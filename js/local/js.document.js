...
$(document).ready(function(){
	...
	//MINOR
		...
		$('#trad-langs').chosen().change(function() {
			...
			//On poste
			$.post(BASEurl+'./ajax/trad.php?sd=1&text='+$('#textid').val()+'&lage='+$(this).val(), function(data) { $('#traduction-list').html(data); ; 
			$(document).ratyP('.tradnote', 1);});
		});
		//On charge au lancement de la page
		$.getJSON(BASEurl+"./json/json.trad.lang.php?sd=1&text="+$('#textid').val(), null, function(data) { $("#trad-langs").get(0).length = 0; $("#trad-langs").addItems(data); $("#trad-langs").trigger("liszt:updated"); })
	...
	$('#poem .epi-note').live('click', function(e) {
		...
		//On appelle l'html
		$.post(BASEurl+'./ajax/tt.note.php?sd=1&text='+$('#textid').val()+'&part='+idnote, function(data) { 
	...
	//Major-Formulaire
		...
		$('.switch-note-var').live('click', function() {
			...
			if(target=='var') {$.getJSON(BASEurl+"./json/json.manuscrits.php?sd=1&uid="+$('#new-'+target+'-uid').val(), null, function(data) { $("#maninput").get(0).length = 0; $("#maninput").addItems(data); $("#maninput").trigger("liszt:updated"); }) }
			//On switch
			$('#new-'+src).hide();
			$('#new-'+target).show();
		});
		...
		$('#new-note-submit').live('click', function(){
			if( $("#new-note").valid()) {
				$.post(BASEurl+'./ajax/tt.submit.php?sd=1', $('#new-note').serializeArray(), function(data) { 
		...
		$('#new-var-submit').click(function(){
			if( $("#new-var").valid()) {
				$.post('./ajax/tt.submit.php?sd=1', $('#new-var').serialize(), function(data) { 
...
//Dialogs		
	...
	$("#tt-new-manuscrit-submit").click(function() {
		if( $("#tt-new-manuscrit-form").valid()) {
			...
			//On poste
			$.post('./ajax/tt.add.manuscrit.php?sd=1&uid='+$('#textid').val(), $('#tt-new-manuscrit-form').serialize(), function(data) { 
				...
				{ 
					...
					$.getJSON(BASEurl+"./json/json.manuscrits.php?sd=1&uid="+$('#textid').val(), null, function(data) { $("#maninput").get(0).length = 0; $("#maninput").addItems(data); $("#maninput").trigger("liszt:updated"); });
				}
			}); 
		}
	});
	...
	$("#tt-new-trad-submit").click(function() {
		if( $("#tt-new-trad-form").valid()) {
			...
			//On poste
			$.post('./ajax/tt.add.trad.php?sd=1&uid='+$('#textid').val(), $('#tt-new-trad-form').serialize(), function(data) { $('#tt-new-trad-result').html(data); $.closedialog('tt-new-trad'); }); 
		}
	});
	...
	$('#dialog-new-trad-note-submit').click(function(){
		if($("#dialog-new-trad-note-form").valid()) {
			$.post(BASEurl+'./ajax/dialog.add.trad.note.php?sd=1', $('#dialog-new-trad-note-form').serializeArray(), function(data) { $('#dialog-new-trad-note .dialog-result').html(data); $.closedialog('dialog-new-trad-note'); });  
		}
	});
	...
	$('.epi-comment').live('click', function() {
		...
		//On appelle l'html
		$.post(BASEurl+'./ajax/dialog.trad.note.php?sd=1&part='+partid, function(data) { $('#dialog-tradnotes').html(data); extract = $('#dialog-tradnotes .comments-extract').html(); });
	});
	//Réponse : clic
	...
	{
		$.getJSON(BASEurl+'./json/json.vote.php?sd=1&t='+table+'&i='+it, function(json) {
	...
	{
		$.getJSON(BASEurl+'./json/json.vote.php?sd=1&t='+table+'&i='+it, {rate: vote}, function(json) {
			var note = parseInt(json.avg);
			sel.parents().attr("note", note);
			sel.raty('start', note).tri(div+'-global');
		});
